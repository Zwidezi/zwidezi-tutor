  ...
  return (
    <div className="max-w-xl mx-auto bg-white p-8 rounded-[2.5rem] shadow-2xl border border-slate-100 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="space-y-8">
        <div>
          <label className="block text-[10px] font-black text-slate-400 mb-4 text-center uppercase tracking-[0.2em]">
            1. Select Grade {selectedGrade && <span className="text-green-600 ml-1">✓</span>}
          </label>
            ...
            {GRADES.map(grade => (
              <button
                ...
                className={`w-16 h-16 rounded-2xl flex items-center justify-center text-xl font-black transition-all border-2 ${
                  selectedGrade === grade 
                    ? 'bg-green-600 border-green-600 text-white shadow-lg scale-110 ring-4 ring-green-50' 
        ...
        <div>
          <label className="block text-[10px] font-black text-slate-400 mb-4 text-center uppercase tracking-[0.2em]">
            2. Choose Subject {selectedSubject && <span className="text-green-600 ml-1">✓</span>}
          </label>
        ...
        <div className={`p-6 rounded-[2rem] border transition-all ${waEnabled ? 'border-green-200 bg-green-50/50' : 'border-slate-100 bg-slate-50'}`}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-3">
              <div className={`p-2 rounded-lg ${waEnabled ? 'bg-green-600 text-white' : 'bg-slate-200 text-slate-400'}`}>
              ...
              <span className={`font-black uppercase tracking-widest text-[10px] ${waEnabled ? 'text-green-800' : 'text-slate-400'}`}>WhatsApp Sync</span>
            </div>
          ...
          <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
            <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">WhatsApp Phone Number</label>
            <div className="relative">
              <input 
                ...
                className={`block w-full bg-white border ${showError && !phoneNumber ? 'border-red-500 bg-red-50' : 'border-slate-200'} text-slate-900 text-sm p-4 rounded-2xl focus:ring-4 focus:ring-green-500/10 focus:border-green-500 outline-none font-bold transition-all shadow-sm`}
              />
              {isPhoneValid && (
                <div className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-black text-green-600 uppercase tracking-widest">🇿🇦 READY</div>
              )}
            </div>
          </div>
        </div>
        ...
        {showError && (!isGradeSelected || !isSubjectSelected || !phoneNumber) && (
          <div className="p-4 bg-red-50 border border-red-100 rounded-2xl flex items-center gap-3 text-red-600 text-xs font-black uppercase tracking-widest animate-in fade-in slide-in-from-top-1">
        ...
        <Button 
          ...
          className={`w-full py-5 rounded-[1.5rem] text-lg font-black transition-all duration-300 ${isFormComplete ? 'shadow-xl shadow-green-100 scale-105 active:scale-95' : 'opacity-50 grayscale cursor-not-allowed'}`}
          variant="primary"
